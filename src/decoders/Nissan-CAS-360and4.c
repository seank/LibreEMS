...
#define NISSAN_360_4_C

#include "../inc/freeEMS.h"
#include "../inc/interrupts.h"
#include "inc/Nissan-CAS-360and4.h"
#include "../inc/decoderInterface.h"
#include "../inc/utils.h"

const unsigned short eventAngles[] = {ANGLE(  0), ANGLE(16), ANGLE(180), ANGLE(348),
                                      ANGLE(360), ANGLE(532), ANGLE(540), ANGLE(716)};
const unsigned char eventValidForCrankSync[] = {0,0,0,0,0,0,0,0}; // This is wrong, but will never be used on this decoder anyway.
const unsigned char windowCounts[] = {176,16,164,12,168,8,172,4};
...
//		DEBUG = windowCounts[currentEvent]; // TODO remove DEBUG
		...
		if((bastardTeeth > MAX_BASTARD_TEETH) || (bastardTeeth < -MAX_BASTARD_TEETH)){
			...
			if((KeyUserDebugs.currentEvent % 2) == 1){ /* if we captured on a rising edge that is to say an evenly spaced edge perform the cacls */
				 ...
				 */
				*ticksPerDegreeRecord = (unsigned short)((ticks_per_degree_multiplier * timeBetweenSuccessivePrimaryPulses) / (180 * ANGLE_FACTOR));
				// instead of above:
